<template>
  ...
  <div class="flex flex-col absolute -translate-y-2/4 -translate-x-2/4 left-1/2 top-1/2 bg-white-100 rounded-xl w-2/6 h-5/12">
    ...
    <input type = "text" placeholder = "Username" v-model = "username" class = "w-8/12 h-7 mb-2 p-2 border border-grey-200 rounded-md relative -translate-x-2/4 left-1/2 outline-red drop-shadow-sm"/>
    <input type = "password" placeholder = "Password" v-model = "password" class = "w-8/12 h-7 p-2 mb-2 border border-grey-200 rounded-md relative -translate-x-2/4 left-1/2 outline-red drop-shadow-sm"/>
...
export default {
  ...
  methods: {
    ...
    loginAttempt() {
      ...
      this.$http.post(`${serverPath}${apiPath}`, body).then(response => {
        this.$cookies.set('auth-token', response.data.token)
        this.loginCorrect()
      }).catch(error => {
        this.loginIncorrect()
